	...
	Private Static Function _config($group, $name, $fallback=null) {
		...
		if (empty($_config)) {
			...
			//$_config = (array) @parse_ini_file(
				//KRUMO_DIR . 'krumo.ini',
				//true);
				// web root is coded like this in case opencart resides in a different directory than root 
		$web_root = "http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF']);
		$web_root = substr($web_root, 0, strrpos($web_root, "/"));
		$web_root .='/system/helper/krumo/';
		$_config['skin']['selected']='blue';
		$_config['css']['url']=$web_root;
			}
